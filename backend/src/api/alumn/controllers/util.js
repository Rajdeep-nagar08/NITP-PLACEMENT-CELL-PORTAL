...
module.exports = {
    /**
     * Helper function to check if a alumn is eligible for a job
     * 
     * @param {Student} alumn Student data
     * @param {Job} job Job object which has to be checked 
     * @param {[Application]} selected_applications
     * - Array of applications, for which "the current alumn" has already been selected
     * 
     * @notes
     ...
     * - Some fields in alumn collection are relations, so they need to be populated,
     *   eg. alumn.course
     * - selected_applications should contain "all" applications for which the alumn has
     ...
     * - job minimum marks are less than or equal to alumn's marks, X, XII, CPI
     * - job is eligible for alumn's course
     * - job category matches alumn's registerd_for category, eg. Internship/FTE
     ...
     * - job is not already applied by alumn
     * - If job.only_for_ews, then only EWS alumns are eligible
     * - If job.only_for_pwd, then only PWD alumns are eligible
     * - If job.only_for_female, then only Female alumns are eligible
     ...
     * - 4. If alumn receives 2 offers, not eligible for more applications
     *
     * Note: The above checks will also include considering the "placed_status" in alumn
     *
     * @returns {boolean} If the alumn is eligible, this will return true
     */
    async helper_is_job_eligible(alumn, job, selected_applications) {
        ...
        const { id, X_marks, XII_marks, cpi, registered_for, course, placed_status, placed_status_updated, internship_status } = alumn;
        if (!id || !X_marks || !XII_marks || !cpi || !registered_for) {
            throw `Some mandatory parameters not passed, or are null: ${alumn, job}`;
        }
...

        {
            ...
            if (job.category != registered_for) {
                debug_reason("Job category does not match alumn's registered_for");
                return false /* Job's category is not the one alumn registered for */;
            }

            if(job.only_for_ews) {
                if(alumn.category != "ews") {
        ...
        {
            // Check if alumn has already applied to this job
    ...
    if ( job.category == "Internship" ) {
        ...
        if ( job.category == "FTE" ) {
            ...
            // Date at which alumn was first selected in A2 (if any)
            ...
            // Number of applications to A1 jobs created by alumn, AFTER being selected in an A2 job
    ...
    /**
     * Helper Function: To get applications of alumn, to tell which jobs the alumn has applied to,
     ...
     * 
     * @param {string/number} roll - Roll number of the alumn, whose applications are to be fetched 
     * @returns 
     */


    async helper_get_applications(roll) {
        const alumn_self = await strapi.db.query("api::alumn.alumn").findOne({
            where: {
                roll: roll,
            },
            select: ["id", "approved"]
        ...
        if (!alumn_self) {
            return []; // "No alumn found"
        }

        const { id, approved } = alumn_self;
        ...
        const applied_jobs = await strapi.db.query("api::application.application").findMany({
            ...
            populate: ["alumn", "job.company", "job.jaf"]
        });
    ...
    //     const applied_alumns = await strapi.db.query("api::application.application").findMany({
    //         where: {
    //             job: id,
    //         },
    //         populate: ["alumn", "job.company", "job.jaf"]
