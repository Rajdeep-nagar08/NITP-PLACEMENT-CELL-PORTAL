...
module.exports = {
    ...
    get_eligible_jobs, retrieves a list of eligible jobs for a alumn
     user based on various eligibility criteria such as their academic
      ...
      retrieves a list of all jobs based on the category the alumn is
       registered for. Both functions first verify if the user has
        provided a valid bearer token, then retrieve the alumn's information 
    ...
    */
    ...
    /**
     * @description Searches the jobs db to look for eligible jobs for current alumn
     * 
     * @note There's also a similar API for admin, to get eligible jobs for a given roll number
     * @note The main logic, for whether alumn is eligible for a job is in helper_is_job_eligible
     * 
     * @checks
     * - alumn is logged in
     * - alumn record with roll number exists
     * - alumn is approved by admin
     * - alumn is eligible for job (see `helper_is_job_eligible`)
    ...
    async get_eligible_jobs(ctx) {
        ...
        const { id, approved, X_marks, XII_marks, cpi, registered_for } = alumn_self;
        ...
        // Check applications in which alumn has been selected
        ...
        const is_eligible = await Promise.all(eligible_jobs.map(
            async (job) => {
                ...
                return await helper_is_job_eligible(alumn_self, job, selected_applications);
            })
        );
    ...
    /**
 * @description Searches all jobs according to registered_for current alumn
 *
 * @notes
 ...
 */
    async get_all_jobs(ctx) {
        ...
        if (!alumn_self) {
            return ctx.badRequest(null, [{ messages: [{ id: "No alumn found" }] }]);
        }

        const { approved, registered_for } = alumn_self;
     ...
     * @example http://localhost:1337/api/alumn/apply?jobId=2
    ...
    async apply_to_job(ctx) {
       ...
       const alumn_self = await strapi.db.query("api::alumn.alumn").findOne({
        ...
        const { id, approved, resume, resume_link } = alumn_self;
        if (approved !== "approved") {
            return ctx.badRequest(null, [{ messages: [{ id: "Account not approved yet" }] }]);
        }
        ...
        const is_eligible = await helper_is_job_eligible(alumn_self, job, selected_applications);
        if (!is_eligible) {
    ...
    /**
     * @description Searches the applications collection to look for applied jobs for current alumn
     * @example http://localhost:1337/api/alumn/applied-jobs
     * @note requires authentication
